...
const GraphVisualizer = () => {
  ...
  const runBFS = async () => {
    ...
    await explain(
      "BFS starts from source node and explores level by level using a queue (FIFO).",
      600,
    );
  ...
  const runDFS = async () => {
    ...
    const dfsRecursive = async (node: number) => {
      ...
      setActiveNode(
        currentStack.length ? currentStack[currentStack.length - 1] : null,
      );
        ...
        {nodes.map((node) => (
          ...
          >
            <Text
              style={[
                styles.nodeText,
                (activeNode === node.id || visitedNodes.has(node.id)) && {
                  color: "#fff",
                },
              ]}
            >
        ...
        <View style={styles.statusBox}>
          ...
          <Text style={styles.statusText}>
            ...
            <Text style={{ fontWeight: "bold", color: theme.accent }}>
              {activeNode !== null ? activeNode : "None"}
            </Text>
          </Text>
        </View>
          ...
          <View style={styles.queueBox}>
            ...
            <View style={styles.queueList}>
              {stack.map((item, i) => (
                <View
                  key={i}
                  style={[styles.queueItem, { backgroundColor: theme.accent }]}
                >
                  <Text style={styles.queueText}>{item}</Text>
                </View>
              ))}
