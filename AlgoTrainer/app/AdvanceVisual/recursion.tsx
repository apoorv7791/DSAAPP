...
const RecursionVisualizer = () => {
    ...
    const runFactorial = async () => {
        ...
        // Recursive Descent
        const calculate = async (currentN: number, depth: number): Promise<number> => {
            const frameId = Math.random().toString(36).substr(2, 9);
            const newFrame: StackFrame = { id: frameId, n: currentN, depth, isReturning: false };
            ...
            if (currentN <= 1) {
                setLogs(prev => [`Base case reached: factorial(${currentN}) = 1`, ...prev]);
                setStack(prev => prev.map(f => f.id === frameId ? { ...f, result: 1, isReturning: true } : f));
                await delay(800);
                return 1;
            }
            ...
            setStack(prev => prev.map(f => f.id === frameId ? { ...f, result, isReturning: true } : f));
            setLogs(prev => [`factorial(${currentN}) returns ${currentN} * ${subResult} = ${result}`, ...prev]);
            await delay(800);
...
const getStyles = (theme: any) => StyleSheet.create({
    ...
    stackContainer: { flex: 1.2, backgroundColor: theme.bgCard, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: theme.border },
    logContainer: { flex: 1, backgroundColor: theme.bgCard, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: theme.border },
    sectionTitle: { fontSize: 14, fontWeight: 'bold', color: theme.primary, marginBottom: 10, textTransform: 'uppercase' },
    ...
    resultText: { color: '#fff', fontSize: 12, textAlign: 'center', marginTop: 4 },
    emptyText: { color: theme.textSecondary, textAlign: 'center', marginTop: 20, fontSize: 12 },
    logScroll: { flex: 1 },
    logText: { color: theme.text, fontSize: 12, marginBottom: 6, lineHeight: 18 },
});
