...
async function sendRemoteProgress(
  token: string,
...
export function LearningProgressProvider({
  ...
  const persist = useCallback(
    async (next: LearningProgressMap, updatedId?: LearningTopicId) => {
      // Save to local storage
      await saveLearningProgress(next);

      // If logged in, save to backend
      if (isLoggedIn && updatedId && BACKEND_URL) {
        try {
          const token = await getAccessToken();
          if (token) {
